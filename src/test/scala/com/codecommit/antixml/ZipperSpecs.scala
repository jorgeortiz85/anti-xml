...
object ZipperSpecs extends Specification {
  ...
  "zipper updates within '\\' results" should {
    "rebuild updated at one level" in {
      ...
      val book0 = books(0).asInstanceOf[Elem].copy(attrs=Map("updated" -> "yes"))
      val book2 = books(2).asInstanceOf[Elem].copy(attrs=Map("updated" -> "yes"))
      
      val bookstore2: NodeSeq = books.updated(0, book0).updated(2, book2).up    // ensure we have NodeSeq
    ...
    "rebuild updated at two levels" in {
      ...
      val author0 = authors(0).asInstanceOf[Elem].copy(attrs=Map("updated" -> "yes"))
      val author2 = authors(2).asInstanceOf[Elem].copy(attrs=Map("updated" -> "yes"))
      val author3 = authors(3).asInstanceOf[Elem].copy(attrs=Map("updated" -> "yes"))
      
      val bookstore2: NodeSeq = authors.updated(0, author0).updated(2, author2).updated(3, author3).up.up
